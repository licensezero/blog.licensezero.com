...
License Zero with payment metadata, but no new license terms, would amount to little more than a tip pool, a way to send, say, $5 to every maintainer in your `node_modules` with a single command.  The company facilitating those payments would be a regulated money transmitter, since the payment isn't for goods or services.  And I don't expect the command would get run that often, or for meaningful amounts, in aggregate.  Rather, I'd expect highly visible celebration of the exceptional, generous few, contributing to a misleading overall picture.
