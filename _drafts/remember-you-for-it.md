...
Within that walled garden, there's no need for license tooling or standards.  There is only one license to give, and only one license to accept.  Neither are there any responsible parties to abuse.  Every user is as privileged as any other to advance the state of any particular work of art.

There is no gratitude, either.  No reputation.  No means of telling who to support, if supporting any particular kind of development is what you want or need.  Only the platform itself, which charges each by their usage, based only on its costs.
